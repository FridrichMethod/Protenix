...
class TestTorchFromNumpyEquivalence(unittest.TestCase):
    ...
    def _check_equivalence(self, arr):
        ...
        self.assertEqual(
            old.dtype, new.dtype, f"dtype mismatch: {old.dtype} vs {new.dtype}"
        )
        self.assertEqual(
            old.shape, new.shape, f"shape mismatch: {old.shape} vs {new.shape}"
        )
...
class TestVectorizedNearAtoms(unittest.TestCase):
    ...
    def test_equivalence(self):
        ...
        # New: vectorized
        new = np.isin(np.arange(n), near_atom_indices) & is_resolved.astype(bool)
